...
use crate::{utils::unzip_genxml, ControlError, ControlResult, DeviceControl, DeviceIoError};
...
use cameleon_device::CompressionType;
...
/// Discovery information for a GigE device.
pub type DeviceInfo = ack::Discovery;

/// Control channel handle for GigE Vision devices.
...
impl ControlHandle {
    /// Creates a control handle from discovery info and stream parameters.
    ...
    /// Sets heartbeat timeout used while the control channel is open.
    ...
    /// Returns immutable discovery information for this device.
    pub fn device_info(&self) -> &DeviceInfo {
        &self.info
    }

    /// Returns a bootstrap register accessor.
...
/// Access mode used when opening the control channel.
pub enum OpenMode {
    ...
    Control,
    /// Open in monitor mode without control privilege.
    MonitorAccess,
}
        ...
        U: ack::ParseAckData<'a>,
    {
        ...
        loop {
            ...
            if ack.ack_kind() == ack::AckKind::Pending {
                ...
                retry_count -= 1;
                if retry_count == 0 {
                    return Err(ControlError::Io(DeviceIoError::msg(
...
impl DeviceControl for ControlHandleInner {
    ...
    fn read_reg(&mut self, address: u64) -> ControlResult<[u8; 4]> {
        ...
        ack.iter()
            .next()
            .ok_or_else(|| ControlError::Io(DeviceIoError::msg("no entry in `ReadReg` ack packet")))
            .copied()
    }
    ...
    fn genapi(&mut self) -> ControlResult<String> {
        ...
        let url_string = if !capability.is_manifest_table_supported() {
            ...
            let ent = header
                ...
                .ok_or_else(|| {
                    ControlError::Io(DeviceIoError::msg(
